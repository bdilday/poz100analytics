...
get_war_data = function(bbref_df) {
...
analyze2 = function() {
  ...
  ld_table = pl_lkup %>% rename(playerID=bbrefID) %>%
    ...
    tidyr::spread(statname, statval) %>%
    select(playerID,
           name=nameAbbv,
           season=yearID,
           league=lgID, G, PA, H, X2B, X3B, HR, R, RBI, SB, n) %>%
    arrange(-n, season, league, playerID) %>%
    dplyr::select(-playerID, -n)

  ld_table[is.na(ld_table)] = "___"
  tbl1 = ld_table


  ll = lapply(clms[5:9], function(clm) {
    get_leader(clm)
  })
  ...
  ll = lapply(clms[3:9], function(clm) {
    get_leader(clm)
  })
  ...
  ld_table = pl_lkup %>% rename(playerID=bbrefID) %>%
    ...
    select(playerID,
           name=nameAbbv,
           season=yearID,
           league=lgID, H, X2B, X3B, HR, R, RBI, SB, n) %>%
    arrange(-n, season, league, playerID) %>%
    dplyr::select(-playerID, -n)
  ...
  ll = lapply(clms[c(1:2,5:9)], function(clm) {
    get_leader(clm)
  })
  ...
  ld_table = pl_lkup %>% rename(playerID=bbrefID) %>%
    ...
    select(playerID,
           name=nameAbbv,
           season=yearID,
           league=lgID, G, PA, H, X2B, X3B, R, SB, n) %>%
    arrange(-n, season, league, playerID) %>%
    dplyr::select(-playerID, -n)
  ...
  list(tbl1 = tbl1, tbl2=tbl2, tbl3=tbl3, tbl4=tbl4)
}
